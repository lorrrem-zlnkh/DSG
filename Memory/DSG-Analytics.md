...
│   ├── ui-kit/
│   │   └── index.html           # UI Kit — документация компонентов (ветка UI-kit, не в main)
│   └── bot/
...
│
├── .claude/
│   ├── settings.json            # PostToolUse хук: автосинк DSG-Analytics.md → Obsidian
│   └── launch.json              # Конфиг локального превью (node server.mjs, порт 3000)
│
├── Memory/
│   ├── knowledge-base.md        # Актуальная база знаний проекта
│   └── DSG-Analytics.md         # Этот файл (синкается в Obsidian при изменении)
│
...
**Ветки staging:** catalog-update, digest-rebuild, UI-kit + любые non-main
...
UI-kit ────────────────► GitHub Pages (staging)  ← документация UI Kit
...
| UI Kit (`/ui-kit/`) | ✅ Сверстан, ветка UI-kit (не в main) |
