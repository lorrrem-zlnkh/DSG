...
**Отправка карточек (init_draft):** `sendCard()` — пауза 1000мс между сообщениями
(лимит Telegram на чат ≈1/сек, чтобы не копить флуд) + ретрай 429 (до 5 попыток,
ждём `retry_after` ≤10с) + повторный проход по не дошедшим карточкам до 3 раундов
(пауза 2с); при ошибке HTML-разбора (`isParseError`) фолбэк на обычный текст
(`renderCardPlain`) — НЕ при флуде/сети. href только для http(s)-URL, кавычки экранируются (`attrEsc`).
...
**Пуш-уведомления:** шапка и все карточки уходят с `disable_notification=true` (тихо) —
единственный пуш даёт финальное сообщение с панелью («Дайджест №N готов к модерации»),
когда все карточки уже добавлены.
