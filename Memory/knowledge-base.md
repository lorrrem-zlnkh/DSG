...
**Отправка карточек (init_draft):** `sendCard()` — пауза 700мс между сообщениями (ниже
флуд-лимита Telegram на чат) + ретрай 429 (до 5 попыток, ждём `retry_after` ≤10с) +
повторный проход по не дошедшим карточкам (пауза 2с); при ошибке HTML-разбора фолбэк на
обычный текст (`renderCardPlain`). href только для http(s)-URL, кавычки экранируются (`attrEsc`).
