...
const FLOATING_SOURCES = new Set(["typejournal", "tilda-education", "alistapart", "apple-design", "apple-events"]);
...
async function requestDigestFromOpenAI(digestMeta, posts) {
  ...
  const systemPrompt =
                ...
                "\n\n" +
                "ЭТАЛОННЫЕ ПРИМЕРЫ summary (точно такой стиль нужен):\n" +
                "• «С ростом возможностей нейросетей многие дизайнеры начинают включать их в свои рабочие процессы, используя ИИ для создания ТЗ и прототипов. Это значительно облегчает проверку гипотез без привлечения разработчиков. Вопрос заключается в том, как именно изменить актуальные процессы с учётом новых технологий.»\n" +
                "• «Статья предлагает визуальный чек-лист для создания грамотного UX-дизайна форм. В ней представлены примеры успешных и неудачных решений, что поможет дизайнерам избежать распространённых ошибок при работе с формами ввода. Материал может значительно упростить процесс проектирования.»\n" +
                "• «Статья рассказывает о создании доступных и персонализированных цветовых схем, отражающих иерархию продукта и его бренд. В ней рассмотрены рекомендации по комбинированию цветов для достижения максимальной эффективности. Этот подход помогает дизайнерам лучше передавать эмоциональное и визуальное восприятие продукта.»\n" +
                "• «В статье рассматриваются традиции использования различных единиц измерения в разных странах. Обсуждается необходимость адаптации дизайна к культурным особенностям локализаций. Поднимаются вопросы интернационализации интерфейсов.»\n\n" +
                "ЗАПРЕЩЁННЫЕ шаблоны — никогда не использовать:\n" +
                "— «Этот материал будет полезен тем, кто...» / «Эта статья будет интересна...» — не писать.\n" +
                "— «Автор делится опытом...» как главная мысль без конкретики — не писать.\n" +
                "— «В статье обсуждаются...» без указания чего именно — не писать.\n" +
                "— «Данная информация актуальна для...» — не писать.\n" +
                "— Любое завершение третьего предложения фразой о целевой аудитории без конкретного тезиса — не писать.\n\n" +
                ...
                "Используй принятые русские термины, а не кальки-англицизмы: «гестуры» — это жесты управления экраном, пиши «жесты»; при необходимости поясни термин в скобках (например, «жесты (gestures)»). " +
